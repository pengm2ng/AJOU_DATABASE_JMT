...
select place_nm, total_amt 
from bizplace as biz , (select biz_reg_no, sum(expendtr_rsltn_amt) as total_amt
		        from expendtrexcut
...
from deptdiv, govofcdiv
where deptdiv.dept_div_cd = govofcdiv.dept_div_cd and deptdiv.dept_div_nm = '직속기관' 

//2-3. 검색설정 - 부서구분이 직속기관일때 관서는 수원소방서일때 실국의 종류는 어떤것이 있는가?
select hgdept_div_nm
from govofcdiv, hgdeptdiv
where hgdeptdiv.govofc_div_cd = govofcdiv.govofc_div_cd and govofcdiv.govofc_div_nm = '수원소방서'  

//2-4. 검색설정 - 부서구분이 직속기관이고 관서는 수원소방서이고 실국은 소방서일때 부서의 종류는 어떤것이 있는가?
select dept_nm
from dept, hgdeptdiv
where dept.hgdept_div_cd = hgdeptdiv.hgdept_div_cd and hgdeptdiv.hgdept_div_nm = '소방서'
               											 ...
               											 (SELECT dept_cd_nm
                											 ...
                											 WHERE Dept.hgdept_div_cd =
                                 											(SELECT hgdept_div_cd
                                 													 FROM HgdeptDiv
                                 												 WHERE hgdeptdiv.hgdept_div_nm = '경기도인재개발원') and 6<EXTRACT(MONTH from paymnt_command_de) and EXTRACT(MONTH from paymnt_command_de)<8 )
...
where biz_reg_no = (SELECT biz_reg_no
	            from bizplace
