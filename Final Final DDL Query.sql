...
(
    organization_id serial     not null
        constraint organization_pk
            primary key,
    dept_div_cd     varchar(2) not null
        constraint organizationchart_deptdiv_fk
            references "DeptDiv"
            on update cascade on delete cascade,
    govofc_div_cd   varchar(4) not null
        constraint organizationchart_govofcdiv_fk
            references "GovofcDiv"
            on update cascade on delete cascade,
    hgdept_div_cd   varchar(4) not null
        constraint organizationchart_hgdeptdiv_fk
            references "HgdeptDiv"
            on update cascade on delete cascade,
    dept_cd_nm      varchar(7) not null
        constraint organizationchart_dept_fk
            references "Dept"
            on update cascade on delete cascade
);

comment on table "OrganizationChart" is '조직도';

comment on column "OrganizationChart".dept_div_cd is '부서구분코드';

comment on column "OrganizationChart".govofc_div_cd is '관서구분코드';

comment on column "OrganizationChart".hgdept_div_cd is '실국코드';

comment on column "OrganizationChart".dept_cd_nm is '부서코드';

create sequence "OrganizationChart_organization_id_seq";
